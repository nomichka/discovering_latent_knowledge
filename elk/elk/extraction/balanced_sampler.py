...
class FewShotSampler:
    ...
    def __init__(
        ...
        corrupt_prob: Optional[float] = 0.0
    ):
    ...
    def __iter__(self) -> Iterator[list[dict]]:
        ...
        for sample in cycle(self.dataset):
            label = sample[self.label_col]
            # Corrupt the sample label with probability corrupt_prob
            if random.random() <= self.corrupt_prob:
                label = int(1 - label)
                corrupt_sample = copy.deepcopy(sample)
                corrupt_sample[self.label_col] = label
                if label == 0:
                    neg_buf.append(corrupt_sample)
                elif label == 1:
                    pos_buf.append(corrupt_sample)
            else:
                if label == 0:
